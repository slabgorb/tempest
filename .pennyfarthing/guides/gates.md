...
| **skill-attested** | `gates/skill-attested.md` | Verify required superpowers skills have been invoked and attested in session `<skills-invoked>` | SPDD workflow composite gates (spdd-red-exit, spdd-green-exit) |
...
### SPDD Composite Gates

The `spdd` (Superpower Driven Development) workflow uses two composite gates that combine an existing artifact check with skill attestation:

- **`spdd-red-exit`** = `tests-fail` + `skill-attested(test-driven-development)`
- **`spdd-green-exit`** = `dev-exit` + `skill-attested(test-driven-development, verification-before-completion, requesting-code-review)`
